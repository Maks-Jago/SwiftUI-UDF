...
final class NotificationQueueTests: XCTestCase {
    // MARK: - Test Helpers
    private func createToastNotification(_ message: String, duration: TimeInterval = 2.0) -> NotificationType {
        let config = ToastConfiguration(defaultDuration: duration)
        return .info(message, style: .toast(config))
    }
    
    private func createCustomToastNotification(_ title: String, duration: TimeInterval = 2.0) -> NotificationType {
        let config = ToastConfiguration(defaultDuration: duration)
        let content = NotificationContent(title)
    ...
    func test_QueueManager_EnqueueSingleToast() {
        let queueManager = ToastQueueManager()
        let toast = createToastNotification("Test Toast")
    ...
    func test_SequentialMode_QueuesToastsInOrder() {
        ...
        queueManager.enqueue(createToastNotification("Toast 1"))
        queueManager.enqueue(createToastNotification("Toast 2"))
        queueManager.enqueue(createToastNotification("Toast 3"))
    ...
    func test_StackedMode_RespectsMaxStackLimit() {
        ...
        queueManager.enqueue(createToastNotification("Toast 2"))
        queueManager.enqueue(createToastNotification("Toast 3"))
        queueManager.enqueue(createToastNotification("Toast 4"))
    ...
    func test_QueueCapacity_RemovesOldestWhenFull() {
        ...
        queueManager.enqueue(createToastNotification("Toast 2"))
        queueManager.enqueue(createToastNotification("Toast 3"))
        queueManager.enqueue(createToastNotification("Toast 4")) // Should remove oldest queued
    ...
    func test_AutoDismiss_RemovesToastAfterDuration() async {
        ...
        let shortToast = createToastNotification("Short Toast", duration: 0.05)
    ...
    func test_AutoDismiss_DisabledWithZeroDuration() async {
        ...
        let permanentToast = createToastNotification("Permanent Toast", duration: 0)
    ...
    func test_QueueInfo_ProvidesAccurateStatistics() {
        ...
        queueManager.enqueue(createToastNotification("Toast 3"))
        queueManager.enqueue(createToastNotification("Toast 4"))
        queueManager.enqueue(createToastNotification("Toast 5"))

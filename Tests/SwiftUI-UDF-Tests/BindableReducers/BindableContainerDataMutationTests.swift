...
final class BindableContainerDataMutationTests: XCTestCase {
    ...
    func test_WhenMutateBindableForm_OnleConcreteInstanceOfBindableFormShouldBeUpdated() async throws {
        ...
        var bindedReducersFormCount = try await unwrapAsync(await store.state.itemsForm).reducers.count
        XCTAssertEqual(bindedReducersFormCount, 0)

        var bindedReducersFlowCount = try await unwrapAsync(await store.state.itemsFlow).reducers.count
        ...
        bindedReducersFormCount = try await unwrapAsync(await store.state.itemsForm).reducers.count
        XCTAssertEqual(bindedReducersFormCount, 2)

        bindedReducersFlowCount = try await unwrapAsync(await store.state.itemsFlow).reducers.count
        ...
        _ = try await unwrapAsync(await store.state.itemsForm[.init(value: 2)]?.item)
        ...
        let itemsCount = try await unwrapAsync(await store.state.itemsForm[.init(value: 2)]).paginator.items.count
    ...
    func test_WhenBindableActionDispatched_StorageShouldReceiveOriginalAction() async throws {
        ...
        let itemsForm1 = try await unwrapAsync(await store.state.itemsForm[.init(value: 1)])
        XCTAssertFalse(itemsForm1.paginator.items.isEmpty)

        let itemsForm2 = try await unwrapAsync(await store.state.itemsForm[.init(value: 2)])

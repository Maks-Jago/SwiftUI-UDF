...
/// Defines the icon representation for toast notifications.
///
/// `ToastIcon` provides flexible icon options for toast notifications, supporting
...
public enum ToastIcon: Hashable {
    ...
    /// No icon displayed in the toast notification.
    /// 
    ...
    /// .image("achievement-trophy")     // Achievement notification icon
    /// ```
    /// 
    /// ## Requirements:
    /// - Image must exist in the app bundle
    /// - Consider providing @2x and @3x variants for different screen densities
    /// - Ensure appropriate contrast for notification backgrounds
    /// - Recommended size: 20-24pt at @1x resolution
    case image(String)
    
    /// A custom SwiftUI view as an icon.
    /// 
    /// Displays any arbitrary SwiftUI view as the notification icon, providing

...
/// The main view component for rendering toast notifications.
///
/// `ToastView` is responsible for displaying toast notifications with full visual
...
/// SwiftUI views, providing a flexible foundation for rich notification experiences.
///
/// ## Features:
...
/// ```
///
/// **Note**: This view is typically used internally by the notification system
...
struct ToastView: View {
    ...
    /// Whether this notification has custom SwiftUI content.
    ///
    /// Custom content notifications can provide completely custom views
    ...
    ///
    /// This contains custom views for notifications that need
    ...
    // The icon to display for this toast notification.
    ///
    /// Returns the custom icon if specified in the notification content,
    /// otherwise falls back to semantic defaults based on the notification category.
    ...
    var toastIcon: ToastIcon {
        ...
        case .info:
            return .info
        case .custom:
            return .none // Custom notifications without explicit icons get no default
        }
    }
    
    /// The main message text to display in the toast.
    ///
    /// Extracts the appropriate message content from the notification type,
    /// handling both simple message notifications and complex content notifications.
    ...
    ///
    /// Extracts action buttons from custom content notifications.
    /// Simple message notifications don't have action buttons.
    var actionButtons: [NotificationButton] {
        guard let content = toast.content else { return [] }
        return content.actions.compactMap { $0 as? NotificationButton }
    }
...
private extension ToastView {
    ...
    /// from the toast notification without dismissing it first.
    ///
    /// ## Button Behavior:

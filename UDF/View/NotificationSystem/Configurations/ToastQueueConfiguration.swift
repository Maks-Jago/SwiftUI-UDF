...
///
/// `ToastQueueConfiguration` controls how multiple toast notifications are handled,
/// including display mode (sequential vs stacked), queue limits, and overflow behavior.
/// This enables sophisticated toast management for applications with frequent notifications.
///
/// ## Usage:
/// ```swift
...
public struct ToastQueueConfiguration: Hashable, Sendable {
    ...
    public enum DisplayMode: Equatable, Sendable {
        ...
        /// to each other. Enables quick scanning of multiple notifications
    ...
    /// in sequential mode. Provides breathing room between notifications.
    /// - Default: 0.2 seconds
...
public extension ToastQueueConfiguration {
    ...
    /// Provides clean, focused notification presentation.
    static let sequential = ToastQueueConfiguration(
        displayMode: .sequential
    )
    
    /// Stacked configuration for high-frequency notifications.
    ///
    ...
    /// High-capacity configuration for notification-heavy applications.
    ///
...
/// Manages the queue and display logic for toast notifications.
///
/// `ToastQueueManager` handles the complex logic of queueing, displaying, and
/// managing multiple toast notifications according to the configured behavior.
...
public class ToastQueueManager: ObservableObject {
    ...
    @Published public private(set) var queuedToasts: [NotificationType] = []
    ...
    ///
    /// - Parameter toast: The toast notification to display.
    public func enqueue(_ toast: NotificationType) {
    ...
    ///
    /// Useful for resetting the notification state or handling app state changes.
...
public struct ToastDisplayInfo: Identifiable, Equatable {
    public let id: UUID
    public let toast: NotificationType

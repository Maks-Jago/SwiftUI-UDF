...
// MARK: - DialogBuilder for Fluent Registration

/// A helper struct for creating dialogs with a fluent API similar to AlertBuilder.
///
/// This provides a bridge between the old AlertBuilder pattern and the new Dialog system,
/// allowing for familiar registration patterns while using modern DialogContent and DialogButton.
public struct DialogBuilder: Sendable {
    private let titleBuilder: @Sendable () -> String
    private let textBuilder: @Sendable () -> String
    private let actionsBuilder: @Sendable () -> [any DialogAction]
    
    /// Creates a DialogBuilder with title, text, and actions.
    ///
    /// - Parameters:
    ///   - title: A closure that returns the dialog title.
    ///   - text: A closure that returns the dialog message text.
    ///   - actions: A closure that builds the dialog actions.
    public init(
        title: @escaping @Sendable () -> String,
        text: @escaping @Sendable () -> String = { "" },
        @DialogActionsBuilder actions: @escaping @Sendable () -> [any DialogAction] = { [] }
    ) {
        self.titleBuilder = title
        self.textBuilder = text
        self.actionsBuilder = actions
    }
    
    /// Creates a DialogBuilder with static title and text.
    ///
    /// - Parameters:
    ///   - title: The dialog title.
    ///   - text: The dialog message text.
    ///   - actions: A closure that builds the dialog actions.
    public init(
        title: String,
        text: String = "",
    ...
    /// Converts this DialogBuilder to a DialogTypeProtocol.
    internal func build() -> any DialogTypeProtocol {
...
// MARK: - Modern Dialog Registration Extension

public extension DialogRegistry {
    /// Registers a dialog using the fluent DialogBuilder pattern.
    ///
    /// This method provides a modern, non-deprecated alternative to AlertBuilder.registerAlert
    /// while maintaining a similar API surface for easy migration.
    ///
    /// - Parameters:
    ///   - id: A unique identifier for the dialog.
    ///   - builder: A closure that returns a DialogBuilder configuration.
    ///
    /// ## Example:
    /// ```swift
    /// DialogRegistry.registerDialog(by: ReportReasonsForm.AlertID.reportSubmitSuccess) {
    ///     DialogBuilder(
    ///         title: { Localizations.alertReportSubmitted() },
    ///         text: { "" }
    ///     ) {
    ///         DialogButton.default(Localizations.commonOk()) {
    ///             globalRouter.back()
    ///         }
    ///     }
    /// }
    /// ```
    static func registerDialog<ID: Hashable & Sendable>(
        by id: ID,
        _ builder: @escaping @Sendable () -> DialogBuilder
    ) {
        register(id: id) {
            builder().build()
        }

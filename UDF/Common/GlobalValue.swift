...
enum GlobalValue {
    ...
    @MainActor private static var values = [String: AnyObject]()
    ...
    @MainActor static func value<T: AnyObject>(for vType: T.Type) -> T {
    ...
    @MainActor static func set<T: AnyObject>(_ value: T) {
        let key = String(describing: T.self)
        values[key] = value
    }
}

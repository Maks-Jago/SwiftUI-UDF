...
public extension EnvironmentStore {
    ...
    func subscribe<M: Middleware<State>>(_ middlewareType: M.Type, environment: M.Environment) where M.State == State, M: EnvironmentMiddleware {
        self.subscribe { store in
            middlewareType.init(store: store, environment: environment)
        }
    }

    func subscribe<M: Middleware<State>>(_ middlewareType: M.Type, on queue: DispatchQueue) where M.State == State, M: EnvironmentMiddleware {
        if ProcessInfo.processInfo.xcTest {
            self.subscribe { store in
    ...
    func subscribe<M: Middleware<State>>(_ middlewareType: M.Type, environment: M.Environment, on queue: DispatchQueue) where M.State == State, M: EnvironmentMiddleware {
        self.subscribe { store in
    ...
    func subscribeAsync<M>(_ middlewareType: M.Type, onSubscribe: @escaping () -> Void = {}) where M : Middleware, M: EnvironmentMiddleware, State == M.State {
        let environment = if ProcessInfo.processInfo.xcTest {
            M.buildTestEnvironment(for: store)
        } else {
            M.buildLiveEnvironment(for: store)
        }

        subscribeAsync(middlewareType, environment: environment, onSubscribe: onSubscribe)
    }

    func subscribeAsync<M>(_ middlewareType: M.Type, onSubscribe: @escaping () -> Void = {}) where M : Middleware, State == M.State {
        Task(priority: .userInitiated) {
            await self.store.subscribe(middlewareType.init(store: self.store))
            onSubscribe()
        }
    }
    ...
    func subscribeAsync<M>(_ middlewareType: M.Type, environment: M.Environment, onSubscribe: @escaping () -> Void = {}) where M: Middleware, State == M.State, M: EnvironmentMiddleware {
        Task(priority: .userInitiated) {
            await self.store.subscribe(middlewareType.init(store: self.store, environment: environment))
            onSubscribe()
        }
    }
    ...
    func subscribeAsync<M>(_ middlewareType: M.Type, on queue: DispatchQueue, onSubscribe: @escaping () -> Void = {}) where M: Middleware, State == M.State, M: EnvironmentMiddleware {
        ...
        Task(priority: .userInitiated) {
            await store.subscribe(middlewareType.init(store: self.store, environment: environment, queue: queue))
            onSubscribe()
        }
    }
    ...
    func subscribeAsync<M>(_ middlewareType: M.Type, on queue: DispatchQueue, onSubscribe: @escaping () -> Void = {}) where M: Middleware, State == M.State {
        Task(priority: .userInitiated) {
            await self.store.subscribe(middlewareType.init(store: store, queue: queue))
            onSubscribe()
        }
    }
    ...
    func subscribeAsync<M>(_ middlewareType: M.Type, environment: M.Environment, on queue: DispatchQueue, onSubscribe: @escaping () -> Void = {}) where M: Middleware, State == M.State, M: EnvironmentMiddleware {
        Task(priority: .userInitiated) {
            await self.store.subscribe(middlewareType.init(store: self.store, environment: environment, queue: queue))
            onSubscribe()
        }
    }
    ...
    func subscribe(@MiddlewareBuilder<State> build: @escaping (_ store: any Store<State>) -> [MiddlewareWrapper<State>]) {
        Task.synchronous(priority: .userInitiated) {
            await self._subscribe(build: build)
        }
    }

    private func _subscribe(@MiddlewareBuilder<State> build: @escaping (_ store: any Store<State>) -> [MiddlewareWrapper<State>]) async {
        await self.store.subscribe(
